populate_datasci_user_tables <- function(connection) {
  ...
  #ETL for external_account_id and email
  ...
  ids$email <- regexpr('token_email: [[:print:]]+',ids$event_data)
  ids$email <- substr(ids$event_data,ids$email,ids$email + attr(ids$email,"match.length"))
  ids$email <- gsub('token_email: |\n', '', ids$email)
  ids$email <- ifelse(ids$email == '', NA, ids$email)
  ...
  #Keep only unique triads of id 
  ids <- ids[!is.na(ids$external_account_id) & !is.na(ids$acting_user_id) & !is.na(ids$email),]
  ids <- ids[!duplicated(ids$acting_user_id) & !duplicated(ids$external_account_id) & !duplicated(ids$email),
            c('acting_user_id', 'external_account_id', 'email')]
  ...
  if (nrow(unmatched_events) > 0) {
    unmatched_events$quarantine_reason <- 'both external_account_id and acting_user_id unmatched'
  ...
  if (count > 0) {
    user_event_types <- dbGetQuery(connection, "SELECT distinct (acting_user_id, external_account_id, email)
                                                , acting_user_id, external_account_id, email, type
                                                FROM datasci.user_events")
    duplicate_events_new <- merge(data[,event_cols], user_event_types, by = c('acting_user_id', 'external_account_id', 'email', 'type'), all = FALSE)
  ...
  data <- rbind.fill(data, no_event)
    ...
    lost_user_events <- event_data[!(event_data$email %in% users$email),]
