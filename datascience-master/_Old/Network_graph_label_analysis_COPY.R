...
#tmp <- data[data$external_account_id == 128196, c('external_account_id', 'message_id', 'from', 'to', 'cc','labels', 'date')]
...
#print(unique(x[, 'external_account_id']))
	...
	tmp <- ldply(apply(tmp, 1, function(x) {
		...
		data.frame(message_id = unique(x['message_id']), to = to[!is.na(to)], from = unique(x['from']), from_ego = ifelse(unique(x['from']) %in% user_email, 1, 0))
		}), rbind)
	...
	tmp$to_alter <- 0
	tmp$to_alter[tmp$to == tmp$alter] <- 1
	tmp$to_alter2d <- 0
	...
	tmp$msgs_per_day <- length(unique(x[,'message_id']))/as.numeric(difftime(max(x[,'date']), min(x[,'date']), units = 'days'))
...
#tmp <- data[data$external_account_id == 128184 & data$alter == 'lharrison@myriversidehome.com',
#	c('external_account_id', 'alter', 'message_id', 'date', 'labels', 'to_ego', 'to_alter', 'to_alter2d', 'n_attachments', 'attachment_names', 'msgs_per_day')]
...
alters <- ldply(lapply(split(data, data[, c('external_account_id', 'alter')], drop = T), function(x) {
	...
	ulab <- ifelse(any(grepl('Label_', tmp$labels)), 1, 0)
	...
	ct_pdfs <- sum(!is.na(regexpr('.pdf',unlist(strsplit(tmp$attachment_names, '",attachment;')))))
	...
	data.frame(external_account_id = unique(x[,'external_account_id']),
				...
				ct_pdfs = ct_pdfs,
				cat_personal_label = cat_personal_label,
				cat_promotions_label = cat_promotions_label,
				cat_social_label = cat_social_label,
				cat_updates_label = cat_updates_label,
				cat_forums_label = cat_forums_label,
...
alters_train[, scale_vars] <- as.data.frame(sapply(seq_along(scale_vars), function(x) {
	(alters_train[, scale_vars[x]] - alters_scaling_table['mean', scale_vars[x]])/alters_scaling_table['sd', scale_vars[x]]
	}))
...
logit_ulab_fp <- glm(ulab ~ ct_to_ego + fracpoly1_ct_to_ego + fracpoly2_ct_to_ego + ct_to_alter + fracpoly1_ct_to_alter + fracpoly2_ct_to_alter + 
				ct_to_alter2d + attachments_dummy + pdf_dummy + ct_attachments + ct_pdfs +
				cat_personal_label + cat_promotions_label + cat_social_label + cat_updates_label + cat_forums_label + important_label +
				msgs_per_day + 
...
logit_ulab_fp <- glm(ulab ~ fracpoly1_ct_to_ego + fracpoly2_ct_to_ego + ct_to_alter + fracpoly2_ct_to_alter + 
				attachments_dummy + pdf_dummy + ct_attachments + ct_pdfs +
				cat_personal_label + cat_promotions_label + cat_social_label + cat_updates_label + important_label +
				msgs_per_day + 
				multi_msg + multi_msg:gaps_mean + 
				multi_msg:fracpoly1_gaps_sd + multi_msg:fracpoly2_gaps_sd + 
				multi_msg_to + multi_msg_to:gaps_to_sd + multi_msg_to:fracpoly1_gaps_to_mean +
				multi_msg_to:fracpoly2_gaps_to_mean + multi_msg_to:fracpoly1_gaps_to_sd + 
				multi_msg_from:gaps_from_mean + multi_msg_from:gaps_from_sd,
				data = alters_train, family = binomial(link = 'logit'))
...
logit_ulab_fp_mc <- logit_ulab_fp
alters_test_mc <- alters_test
alters_scaling_table_mc <- alters_scaling_table

model_storage(model_obj = logit_ulab_fp_mc, location_folder = "contact_sf", model_script = "Network_graph_label_analysis.R",
strip_model_fn = strip_model, model_grain = "contact", model_response = "Binary", model_outcome = "binary", model_type = "Logistic Regression",
model_test = alters_test_mc, model_test_cols = c('pred', 'ulab'))

model_data <- dbGetQuery(connection, "SELECT * from datasci_modeling.models")

ensemble <- 'Multi-Contact SF Logit Ensemble 1'
model_id <- model_data$id[model_data$model_name == paste('logit_ulab_fp_mc', '_stripped', sep = '')]
model_weight_type <- 'AUC'
active <- TRUE
model_threshold_type <- 'f2'
ensemble_data <- cbind(ensemble, model_id, model_weight_type, active, model_threshold_type)
...
#tmp <- data[data$external_account_id == 128186, c('external_account_id', 'message_id', 'from', 'to', 'cc', 'from_ego', 'to_ego', 'to_alter', 'to_alter2d','labels', 'date')]
...
sets$set <- ifelse(sets$rand < 0.5, 'train', ifelse(sets$rand >= 0.9, 'validation', 'test'))
...
mfp(ulab_shared ~ fp(cooc_z_alt, df = 4), dyads[dyads$set == 'train',], family = 'binomial')

dyads$fracpoly1_cooc <- ((dyads$cooc+0.1)/0.1)^-2
dyads$fracpoly2_cooc <- ((dyads$cooc+0.1)/0.1)^-2*log(((dyads$cooc+0.1)/0.1))
dyads$fracpoly1_cooc_z <- (dyads$cooc_z_alt/1000)^-2
dyads$fracpoly2_cooc_z <- (dyads$cooc_z_alt/1000)^-2*log((dyads$cooc_z_alt/1000))

logit_dyads_cooc_z_fp <- glm(ulab_shared ~ cooc_z:a1_label_pred + cooc_z:a2_label_pred + cooc_z + fracpoly1_cooc_z + fracpoly2_cooc_z + a2xa1*a1xa2,
				data = dyads[dyads$set == 'train',], family = binomial(link = 'logit'))

# test model
dyads_coef_mat <- t(as.data.frame(logit_dyads$coefficients))

if(exists(interacts)) {dyads <- dyads[, which(!colnames(dyads) %in% colnames(interacts))]}
...
pred <- pred %*% logit_dyads$coefficients
pred <-  1/(1 + exp(-pred))
dyads_test$pred <- pred

dyads_thresh_test <- thresh_iter(0, 1, 0.01, dyads_test, 'pred', 'ulab_shared')

model_storage(model_obj = logit_dyads_fp, location_folder = "contact_sf", model_script = "Network_graph_label_analysis.R",
...
model_test = dyads_test, model_test_cols = c('pred', 'ulab_shared'))

model_data <- dbGetQuery(connection, "SELECT * from datasci_modeling.models")

ensemble <- 'Contact Dyad SF Logit Ensemble 2'
model_id <- model_data$id[model_data$model_name == paste('logit_dyads_fp', '_stripped', sep = '')]
model_weight_type <- 'AUC'
active <- TRUE
model_threshold_type <- 'f2'
ensemble_data <- cbind(ensemble, model_id, model_weight_type, active, model_threshold_type)
