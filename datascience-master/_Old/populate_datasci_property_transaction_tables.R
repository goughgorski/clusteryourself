populate_datasci_property_transaction_tables <- function(connection) {
  ...
  properties_tx_mu_ev_table_exists <- dbExistsTable(connection, c("datasci","property_transaction_multi_events"))
  properties_tx_table_exists <- dbExistsTable(connection, c("datasci","property_transactions"))
  properties_not_tx_table_exists <- dbExistsTable(connection, c("datasci","property_transactions_not_created"))
  properties_table_exists <- dbExistsTable(connection, c("datasci","properties"))
  
  tables_exist <- c(properties_tx_ti_ev_table_exists, properties_tx_ti_table_exists, properties_tx_ev_table_exists,
   properties_tx_mu_ev_table_exists, properties_tx_table_exists, properties_table_exists)
  required_tables <- c('property_transaction_timeline_events', 'property_transaction_events', 'property_transaction_multi_events',
   'property_transactions', 'properties')
  ...
  print(noquote("Populating Property Transaction Multi Events Table"))  
    ...
    #Check if any new data
    if (nrow(data)==0) {
          stop(print('No new multi event data found.')) 
    }
    ...
    #Shared with user_id
    data <- ldply(apply(data, 1, function(x){
    tmp <- gregexpr('user_id: [[:digit:]]+', x['event_data'])
    tmp <- data.frame('position' = tmp[[1]], 'length' = attr(tmp[[1]], 'match.length'))
    tmp$value <- substr(x['event_data'], tmp$position, tmp$position + tmp$length)
    tmp$value <- gsub('user_id: |\n', '', tmp$value)
    tmp$event_id <- x['id']
    property_transaction_id <- regexpr('property_transaction_id: [[:digit:]]+',x['event_data'])
    property_transaction_id <- substr(x['event_data'],property_transaction_id,property_transaction_id +
     attr(property_transaction_id,"match.length"))
      tmp$property_transaction_id <- gsub('property_transaction_id: |\n', '', property_transaction_id)
      tmp$type <- x['type']
    return(tmp)
    }), rbind)
    ...
    multi_event_data <- data[, event_cols]
    multi_event_data$value <- as.numeric(multi_event_data$value)

    #Check for missing property transactions
    property_transactions <- dbGetQuery(connection,"SELECT * FROM datasci.property_transactions")
    lost_property_transaction_multi_events <- data[!(data$property_transaction_id %in% property_transactions$property_transaction_id),]
    if(nrow(lost_property_transaction_multi_events) > 0){
    lost_property_transaction_multi_events$quarantine_reason <- 'originating event not found'
    print(noquote(paste(nrow(lost_property_transaction_multi_events), "transaction multi events found without a creation event.")))
    } else {print(noquote("0 transaction multi events found without a creation event."))}

    #Check for missing shared with user_id
    lost_shared_with_multi_events <- data[data$value == '',]
    if(nrow(lost_shared_with_multi_events) > 0){
    lost_shared_with_multi_events$quarantine_reason <- 'shared with user_id not found'
    print(noquote(paste(nrow(lost_shared_with_multi_events), "transaction multi events found without a shared with user_id.")))
    } else {print(noquote("0 transaction multi events found without a shared with user_id."))}

    #Exclude quarantined events
    quarantined_multi_events <- rbind(lost_property_transaction_multi_events, lost_shared_with_multi_events)
    multi_event_data <- multi_event_data[!(multi_event_data$event_id %in% quarantined_multi_events$event_id),]
    ...
    #Write Quarantined Property Transaction Event Data
    quarantined_multi_events <- quarantined_multi_events[,c(event_cols,'quarantine_reason')]
    print(noquote("Quarantined Multi Events:"))
    print(table(quarantined_multi_events$type))

    dbWriteTable(connection, c('datasci','quarantined_property_transaction_multi_events'), value = quarantined_multi_events, overwrite = FALSE, append = TRUE, row.names = FALSE)

    #Model Scores

    #Scores Below Threshold
    #Query to get new data for property_transaction
    print(noquote("Getting new data from for not created events from production events table"))
    data <- dbGetQuery(connection, "SELECT * FROM datasci.vw_property_transactions_not_created_queue")
    ...
    #Write Property Transaction Event Data
    dbWriteTable(connection, c('datasci','property_transactions_not_created'), value = event_data, overwrite = FALSE, append = TRUE, row.names = FALSE)
